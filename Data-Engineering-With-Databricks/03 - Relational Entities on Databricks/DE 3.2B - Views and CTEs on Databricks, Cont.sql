...
-- MAGIC %md With the Notebook's state reset, we need to re-initialize some of our lesson-specific configuration.
-- MAGIC 
...
-- MAGIC %md But, we do need to configure this session to use our database by default.
...
-- MAGIC %md Finally, run the following two cells to confirm that:
...
-- MAGIC %md As mentioned previously, temp views are tied to a Spark session and as such are not accessible...
...
-- MAGIC %md Global temp views are "lost" when the cluster is restarted.
-- MAGIC 
